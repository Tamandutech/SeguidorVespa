...
/// Sensor types.
enum class QTRType : uint8_t { Undefined, RC, Analog, AnalogESP, Multiplexer };
...
public:
  ...
  /// \brief Specifies that the sensor type is multiplexer.
  ///
  /// Call this function to set up Multiplexer-type sensors.
  void setTypeMultiplexer();

  void setSensorPins(const uint8_t analogPin, uint8_t sensorCount,
                     uint8_t *multiplexerPins, uint8_t multiplexerPinCount);
...
private:
  ...
  uint8_t                  *_sensorPinsMultiplexer = nullptr;
