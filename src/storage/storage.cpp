...
bool Storage::is_mounted() {
  std::lock_guard<std::mutex> lock(fs_mutex);
  return mount_status;
}

esp_err_t Storage::mount_storage(std::string mount_path) {
  std::lock_guard<std::mutex> lock(fs_mutex);
  ...
  esp_err_t mount_result = esp_vfs_fat_spiflash_mount_rw_wl(
      mount_point.c_str(), "storage", &fat_mount_config, &wear_leveling_handle);
...
esp_err_t Storage::list_files() {
  ...
  if(directory_handle) {
    ESP_LOGD(logger_tag.c_str(), "Listing directory %s:", mount_point.c_str());
...
esp_err_t Storage::save_data(std::string file_path, char *data_buffer,
                             size_t data_size, const char *mode) {
  std::lock_guard<std::mutex> lock(fs_mutex);
  if(!mount_status) {
    ESP_LOGD(logger_tag.c_str(), "FATFS not mounted.");
    return ESP_FAIL;
...
esp_err_t Storage::load_data(std::string file_path, char *data_buffer,
                            size_t data_size) {
  std::lock_guard<std::mutex> lock(fs_mutex);
  if(!mount_status) {
    ESP_LOGD(logger_tag.c_str(), "FATFS not mounted.");
...
esp_err_t Storage::delete_data(std::string file_path) {
  std::lock_guard<std::mutex> lock(fs_mutex);
  if(!mount_status) {
    ESP_LOGD(logger_tag.c_str(), "FATFS not mounted.");
    return ESP_FAIL;
...
bool Storage::file_exists(std::string file_path) {
  std::lock_guard<std::mutex> lock(fs_mutex);
  if(!mount_status) {
    return false;
  }
...
esp_err_t Storage::write(DataType data_value, std::string file_path) {
  return save_data(file_path, reinterpret_cast<char *>(&data_value),
                   sizeof(DataType));
}

template <typename DataType>
esp_err_t Storage::read(DataType &data_value, std::string file_path) {
  return load_data(file_path, reinterpret_cast<char *>(&data_value),
                   sizeof(DataType));
}

template <typename ElementType>
esp_err_t Storage::write_vector(const std::vector<ElementType> &vector_data,
...
template <typename ElementType>
esp_err_t Storage::read_vector(std::vector<ElementType> &vector_data,
                               std::string file_path) {
  std::lock_guard<std::mutex> lock(fs_mutex);
  if(!mount_status) {
    ESP_LOGD(logger_tag.c_str(), "FATFS not mounted.");
...
template esp_err_t Storage::write<ParametersConfig>(ParametersConfig,
                                                    std::string);
template esp_err_t Storage::read<ParametersConfig>(ParametersConfig &,
                                                   std::string);
template esp_err_t Storage::write_vector<MapPoint>(
    const std::vector<MapPoint> &, std::string);
template esp_err_t Storage::read_vector<MapPoint>(std::vector<MapPoint> &,
                                                  std::string);
